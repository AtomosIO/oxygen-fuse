...
func (mnt *Mount) Close() {
	...
	mnt.Conn.Close()
	os.Remove(mnt.Dir)
	log.Printf("Unmounted: %s", mnt.Dir)
}
...
func CreateMountServeOxygenFSInTempDir() (*Mount, error) {
	fs := OxygenFS{
		requests: map[fuse.RequestID]*serveRequest{},
	}

	return MountServeOxygenFSInTempDir(&fs)
}

func MountServeOxygenFSInTempDir(filesys *OxygenFS) (*Mount, error) {
	dir, err := ioutil.TempDir("", "fusetest")
	if err != nil {
		return nil, err
	}
	log.Printf("Mounting: %s", dir)
